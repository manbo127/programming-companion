...
from companion.utils.code_utils import has_code, has_error
...
class MessageClassifier:
    ...
    def classify(
        ...
        # ── 优先级 1: 错误解读 ────────────────────
        # 同时在 text 和 error 中搜索关键词（覆盖纯 error 输入）
        error_search_text = text_lower + " " + (error.lower() if error else "")
        error_score = self._match_score(error_search_text, self.error_keywords)

        # 高置信条件：代码+错误同时存在 / 仅错误 / 多错误关键词
        if result.has_error and (result.has_code or error_score >= 1):
            result.scene = "error"
            result.confidence = 1.0 if result.has_code else 0.9
            result.matched_keywords = ["code+error combined"] if result.has_code else ["error only"]
            return result
